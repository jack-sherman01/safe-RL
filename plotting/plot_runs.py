...
def plot_experiment(experiment, logdir):
    ...
    for fname in os.listdir(logdir):
        if fname == '2023-03-02_00-11-19_SAC_navigation1_Gaussian_RRL_MB':
            continue
    ...
    for alg in experiment_map["algs"]:
        ...
        task_successes_list = np.array(task_successes_list,dtype=object)
        train_violations_list = np.array(train_violations_list,dtype=object)
        ...
        train_rewards_safe_list = np.array(train_rewards_safe_list, dtype=object)

        safe_ratios = (task_successes_list + 1) / (train_violations_list + 1)
        final_ratio = safe_ratios.mean(axis=0, dtype=object)[-1]
...
if __name__ == '__main__':
    ...
    logdir = '/home/hzhang/safe-RL/recovery-rl/navigation1'  # ** insert logdir here **
